...
@Entity
@Table(name = "outward_micr_repair")
public class OutwardMicrRepair {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    // Many Repair Entries -> One Cheque  (two-way: OutwardCheque.micrRepairs)
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "outward_cheque_id", nullable = false)
    private OutwardCheque outwardCheque;

    // Which sub-field was changed: cheque_no | bank_code | city_code | branch_code | transaction_code
