...
public class BatchUploadComposer extends SelectorComposer<Component> {
    ...
    @Wire private Textbox batchIdFilter;
    ...
    public void onProcessBatch() {
        ...
        lastResult = service.processBatchUpload(
            uploadedFile, count, amount.doubleValue(), currentMakerId);
    ...
    private Row buildBatchRow(final OutwardBatch b, boolean isNew) {
        ...
        // Pending = total, Processed = 0 (updated as workflow progresses)
        int total     = b.getChequeCount();
        int processed = 0;
        int pending   = total - processed;

        Row row = new Row();
        if (isNew) row.setSclass("row-new");
    ...
    /** Creates the action button for a batch row based on its status. */
    private Button buildActionBtn(final OutwardBatch b) {
        Button btn = new Button();
        String status = b.getStatus() != null ? b.getStatus() : "";

        switch (status) {
            case "NEEDS_REPAIR":
                btn.setLabel("🔧 MICR Repair");
     ...
     */
    @Listen("onClick = #mismatchRejectBtn; onClick = #mismatchCloseBtn")
            ...
            new EventListener<InputEvent>() {
                @Override public void onEvent(InputEvent e) {
                    applyFilters(e.getValue(), getSelectedStatus(),
                                 batchIdFilter.getValue());
                }
            });

        // Column filter instant
        batchIdFilter.addEventListener(Events.ON_CHANGING,
            new EventListener<InputEvent>() {
                @Override public void onEvent(InputEvent e) {
                    applyFilters(batchSearchBox.getValue(),
                                 getSelectedStatus(), e.getValue());
                }
            });
    ...
    private void applyFilters(String search, String status, String colFilter) {
        ...
        String col  = colFilter != null ? colFilter.trim().toLowerCase() : "";
        ...
        for (Object obj : batchRows.getChildren()) {
            ...
            // Get batch ID from column 1
            String batchId = "";
            String rowStatus = "";
            int idx = 0;
            for (Object cell : row.getChildren()) {
                if (cell instanceof Label) {
                    if (idx == 1) batchId  = ((Label) cell).getValue().toLowerCase();
                    if (idx == 6) rowStatus = ((Label) cell).getValue().toUpperCase()
                                                             .replace(" ", "_");
                }
                idx++;
            }

            boolean matchSearch = srch.isEmpty() || batchId.contains(srch);
            boolean matchStatus = stat.isEmpty() || rowStatus.contains(stat);
            boolean matchCol    = col.isEmpty()  || batchId.contains(col);

            boolean show = matchSearch && matchStatus && matchCol;
            row.setVisible(show);
            if (show) visible++;
        }
