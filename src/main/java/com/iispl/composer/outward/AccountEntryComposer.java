...
/**
 * File : com/iispl/composer/outward/AccountEntryComposer.java Purpose : Account
 * & Amount Entry screen for Maker Outward.
 *
 * CBS Validation Rules: 1. Maker must click "Validate" before clicking "Save &
 * Next" 2. If not validated → Save is blocked with notification 3. If account
 * ACTIVE → Save allowed, CBS holder name stored 4. If account
 * INACTIVE/CLOSED/FROZEN → Save blocked, reject panel auto-opens with
 * appropriate reason 5. If account NOT FOUND → Save blocked, reject panel opens
 * 6. If CBS service error → Retry allowed (save still blocked) 7. Changing
 * account number after validation → resets validation 8. Loading a new cheque →
 * always resets validation
 */
public class AccountEntryComposer extends SelectorComposer<Component> {
	...
	// ── Three views ──
	@Wire
	private Div emptyStateView;
	@Wire
	private Div batchSelectView;
	@Wire
	private Div entryView;

	// ── Batch Select View ──
	@Wire
	private Rows batchSelectRows;

	// ── Pagination ──
	@Wire
	private Div batchPager;
	@Wire
	private Button btnPrevPage;
	@Wire
	private Button btnNextPage;
	@Wire
	private Label batchPagerInfo;

	// ── Entry View — Stats bar ──
	@Wire
	private Label statsBatchId;
	@Wire
	private Label statsTotal;
	@Wire
	private Label statsDoneCount;
	@Wire
	private Label statsRemCount;
	...
	@Wire
	private Button tabFrontBtn;
	@Wire
	private Button tabBackBtn;
	@Wire
	private Div chqFront;
	@Wire
	private Div chqBack;
	@Wire
	...
	@Wire
	private Label chqNoDisplay;
	@Wire
	private Label chqDateDisplay;
	@Wire
	private Label entryStatusBadge;

	// ── Right Panel — Account & CBS Validation ──
	@Wire
	private Textbox accountNoBox;
	@Wire
	private Button validateBtn;
	@Wire
	private Button saveNextBtn;
	@Wire
	private Div valResultDiv;
	@Wire
	private Label valMessageLabel; // success / warning / error message
	@Wire
	private Div valDetailsDiv; // account details panel
	@Wire
	private Label valHolderLabel; // account holder name
	@Wire
	private Label valTypeLabel; // SAVINGS / CURRENT
	@Wire
	private Label valStatusBadge; // ACTIVE / INACTIVE badge
	@Wire
	private Label valIfscLabel; // IFSC code
	@Wire
	private Label valBalanceLabel; // balance

	// ── CBS cache hint ──
	@Wire
	private Div cbsCacheDiv;
	@Wire
	private Label cbsCacheLabel;

	// ── Right Panel — Amount Entry ──
	@Wire
	private Decimalbox amountBox;
	@Wire
	private Textbox chequeDateBox;
	@Wire
	private Textbox amountInWordsBox;

	// ── Right Panel — Payee ──
	@Wire
	private Textbox payeeNameBox;

	// ── Footer ──
	@Wire
	private Div rejectPanel;
	@Wire
	private Listbox rejectReasonBox;
	@Wire
	private Textbox rejectRemarksBox;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		if (batchId != null && !batchId.trim().isEmpty()) {
			...
			if (currentBatch == null) {
				Clients.showNotification("Batch not found: " + batchId, "error", null, "top_center", 3000);
	...
	private void loadBatchSelectView() {
		List<OutwardBatch> batches = entryService.getEntryBatches(currentMakerId);
	...
	private void renderBatchPage() {
		...
		batchPagerInfo.setValue("Page " + (batchPage + 1) + " of " + totalPages + "  (" + totalItems + " batches)");
	...
	public void onNextPage() {
		if (batchDisplayList != null) {
			int totalPages = (int) Math.ceil((double) batchDisplayList.size() / PAGE_SIZE);
	...
	private Row buildBatchSelectRow(int idx, final OutwardBatch b, int pending) {
		...
		row.appendChild(new Label(b.getActualAmount() != null ? "₹" + moneyFmt.format(b.getActualAmount()) : "—"));
		...
		selectBtn.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			@Override
	...
	private void loadEntryView() {
		...
		if (pendingList.isEmpty()) {
			Clients.showNotification("All entries already done for this batch.", "info", null, "top_center", 3000);
			return;
		}
	...
	private void refreshStatsBar() {
		...
		statsTotal.setValue(
				currentBatch.getActualAmount() != null ? "₹" + moneyFmt.format(currentBatch.getActualAmount()) : "—");
		statsDoneCount.setValue(String.valueOf(totalCheques - pendingList.size()));
	...
	private void loadChequeForm(OutwardCheque cheque) {
		...
		chqDateDisplay.setValue(cheque.getChequeDate() != null ? cheque.getChequeDate().toString() : "—");
		...
		if (cheque.getAmount() != null) {
			...
			amountInWordsBox.setValue(AmountToWords.convert(cheque.getAmount().doubleValue()));
		} else {
		...
		chequeDateBox.setValue(cheque.getChequeDate() != null ? cheque.getChequeDate().toString() : "");
	...
	private void loadImages(OutwardCheque cheque) {
		try {
			...
			frontImage.setSrc(
					fp != null && !fp.trim().isEmpty() ? "/imageServlet?path=" + URLEncoder.encode(fp.trim(), "UTF-8")
							: "");

			String bp = cheque.getBackImagePath();
			backImage.setSrc(
					bp != null && !bp.trim().isEmpty() ? "/imageServlet?path=" + URLEncoder.encode(bp.trim(), "UTF-8")
							: "");
		} catch (UnsupportedEncodingException e) {
			System.err.println("AccountEntryComposer → image URL encode failed: " + e.getMessage());
		}
	}
	...
	public void onValidate() {
		...
		if (isBlank(accNo)) {
			Clients.showNotification("Please enter an account number before validating.", "warning", null, "top_center",
					2000);
			return;
		}
		...
		if (cbsCache.containsKey(key)) {
			...
			cbsCacheLabel.setValue("⚡ CBS result loaded from session cache " + "(no network call made).");
	...
	/**
	 * Displays the CBS validation result in the UI. Updates: valResultDiv,
	 * valMessageLabel, valDetailsDiv, valHolderLabel, valTypeLabel, valStatusBadge,
	...
	private void displayCbsResult(CbsValidationResult result) {
		...
		if (result.isFound() && result.isActive()) {
			...
			valMessageLabel.setValue("✓ Account validated — " + safe(result.getAccountHolderName()));
			valMessageLabel.setSclass("val-ok");
			...
			System.out.println("AccountEntryComposer → Validation PASSED: " + result.getAccountHolderName());
			...
			valMessageLabel.setValue("⚠ Account is " + status + ". This cheque must be rejected.");
			...
			System.out.println("AccountEntryComposer → Account is " + status + " — reject required");
			...
			String msg = result.getErrorMessage() != null ? result.getErrorMessage() : "Account not found in CBS.";
			...
			System.out.println("AccountEntryComposer → Validation FAILED: " + msg);
		}
	}

	/**
	 * Opens reject panel and pre-selects the reason code for INACTIVE / CLOSED /
	 * FROZEN accounts.
	 */
	private void openRejectPanelForInactiveAccount(String accountStatus) {
		rejectPanel.setVisible(true);
		rejectRemarksBox.setValue("CBS validation failed — Account status: " + accountStatus);
		...
		Clients.showNotification("Account is " + accountStatus + ". Please reject this cheque using the panel below.",
	...
	private void openRejectPanelForNotFoundAccount() {
		...
		Clients.showNotification("Account not found in CBS. Please reject this cheque.", "warning", null, "top_center",
				4000);
	}
	...
	public void onAmountChange() {
		...
		if (amt != null && amt.compareTo(BigDecimal.ZERO) > 0) {
			amountInWordsBox.setValue(AmountToWords.convert(amt.doubleValue()));
		} else {
			amountInWordsBox.setValue("");
		}
		autoFillingWords = false;
	...
	/**
	 * Shared navigation helper that prompts about unsaved changes before moving
	 * away from the current cheque. Used by the Prev / Next buttons.
	 */
	private void navigateTo(final int newIndex) {
		...
		if (isDirty) {
			Messagebox.show(
					...
					"Unsaved Changes", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION, event -> {
	...
	public void onSaveNext() {
		...
		if (!isValidated) {
			Clients.showNotification(
					"⚠ Account must be validated via CBS before saving. " + "Click the 'Validate' button first.",
		...
		if (cbsResult != null && !cbsResult.isActive()) {
			Clients.showNotification(
					"Account is " + cbsResult.getStatusLabel() + ". This cheque cannot be saved — it must be rejected.",
		...
		if (isBlank(accNo)) {
			Clients.showNotification("Account number is required.", "warning", null, "top_center", 2000);
			return;
		}
		if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
			Clients.showNotification("Amount is required and must be greater than zero.", "warning", null, "top_center",
					2000);
			return;
		}
		if (isBlank(dateStr)) {
			Clients.showNotification("Cheque date is required.", "warning", null, "top_center", 2000);
			return;
		}
		if (isBlank(words)) {
			Clients.showNotification("Amount in words is required.", "warning", null, "top_center", 2000);
			return;
		}
		if (isBlank(payee)) {
			Clients.showNotification("Payee name is required.", "warning", null, "top_center", 2000);
			return;
		}
		...
		if (originalXmlAmount != null && originalXmlAmount.compareTo(BigDecimal.ZERO) > 0
			...
			String msg = String.format("Entered amount ₹%s differs from the XML-parsed amount ₹%s "
					+ "by more than 1%%.%n%nThis could be a data entry error.%n%n" + "Save the entered value anyway?",
					amount.setScale(2, RoundingMode.HALF_UP).toPlainString(),
					originalXmlAmount.setScale(2, RoundingMode.HALF_UP).toPlainString());
			Messagebox.show(msg, "Amount Mismatch Warning", Messagebox.YES | Messagebox.NO, Messagebox.EXCLAMATION,
					event -> {
						if (Messagebox.ON_YES.equals(event.getName())) {
							doSaveCheque(cheque, finalAccNo, finalAmount, finalWords, finalDate, finalPayee);
						}
					});
			return;
		}
	 ...
	 * confirmation are passed. Updates the thumbnail strip and progress bar, then
	 * either advances to the next cheque or finalises the batch.
	 */
	private void doSaveCheque(OutwardCheque cheque, String accNo, BigDecimal amount, String words, String dateStr,
			String payee) {

		// ── Get CBS account holder name to store ──
		String accountHolder = (cbsResult != null && cbsResult.getAccountHolderName() != null
				&& !cbsResult.getAccountHolderName().isEmpty()) ? cbsResult.getAccountHolderName() : "Validated";

		// ── Save to DB ──
		boolean ok = entryService.saveEntry(cheque.getId(), accNo.trim(), accountHolder, amount, words.trim(),
				dateStr.trim(), payee.trim(), currentMakerId);

		if (!ok) {
			Clients.showNotification("Failed to save entry. Please try again.", "error", null, "top_center", 2500);
			return;
		}

		Clients.showNotification("✓ Cheque " + cheque.getChequeNo() + " entry saved.", "info", null, "top_center",
				2000);
		...
		if (entryService.isAllEntriesDone(currentBatch.getId())) {
			...
			if (!"REFER_BACK".equals(currentBatch.getStatus())) {
				...
				Clients.showNotification("All entries done! Batch " + batchId + " submitted to Checker queue.", "info",
				...
				if (remaining == 0) {
					...
					Clients.showNotification(remaining + " referred cheque(s) still pending in MICR Repair module.",
	...
	public void onConfirmReject() {
		if (rejectReasonBox.getSelectedItem() == null
			...
			Clients.showNotification("Please select a rejection reason.", "warning", null, "top_center", 2500);
			return;
		}
		...
		boolean ok = entryService.rejectCheque(cheque.getId(), reason, remarks, currentMakerId);

		if (!ok) {
			Clients.showNotification("Rejection failed. Please try again.", "error", null, "top_center", 2500);
			return;
		}

		Clients.showNotification("Cheque " + cheque.getChequeNo() + " rejected.", "info", null, "top_center", 2000);
		...
		if (entryService.isAllEntriesDone(currentBatch.getId())) {
			...
			if (!"REFER_BACK".equals(currentBatch.getStatus())) {
				...
				System.out.println("AccountEntryComposer → REFER_BACK batch " + currentBatch.getBatchId()
	...
	public void onBack() {
		...
		if (isDirty) {
			Messagebox.show("You have unsaved changes on this cheque.\n\n" + "Leave anyway? Your edits will be lost.",
					"Unsaved Changes", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION, event -> {
						if (Messagebox.ON_YES.equals(event.getName())) {
							isDirty = false;
							doBack();
	...
	/**
	 * cameFromSidebar drives the back navigation: true → user arrived from the
	 * sidebar link → return to the in-page batch selection view. false → user
	 * arrived via the batchId URL parameter (i.e. from the MICR Repair screen's
	 * "Proceed to Account Entry" button) → go back to MICR Repair.
	 */
	private void doBack() {
		if (cameFromSidebar) {
			loadBatchSelectView();
	...
	/**
	 * Shows a YES/NO popup asking the maker to resubmit the batch to Checker. Fires
	 * only when countActiveReferrals() == 0 for a REFER_BACK batch. If both MICR
	 * and Data Entry modules had referred cheques, this popup only appears when the
	 * LAST cheque across BOTH modules is completed.
	 */
	private void showResubmitPopup() {
		...
		Messagebox.show(
				...
				"Resubmit to Checker", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION, event -> {
					if (Messagebox.ON_YES.equals(event.getName())) {
						doResubmit();
					}
				});
	...
	private void doResubmit() {
		if (currentBatch == null)
			return;
		boolean ok = makerOutwardService.resubmitBatch(currentBatch.getId(), currentMakerId, currentMakerName);
		if (ok) {
			Clients.showNotification("Batch " + currentBatch.getBatchId() + " resubmitted to Checker.", "info", null,
					"top_center", 3000);
		} else {
			Clients.showNotification("Resubmit failed. Please try from View Batches.", "error", null, "top_center",
					3000);
		}
	...
	/**
	 * Returns true if the maker-entered amount differs from the XML-parsed amount
	 * by more than 1%.
	 *
	 * Why 1%? Banks tolerate small rounding when re-keying handwritten cheques but
	 * anything beyond that probably indicates a missed/extra digit (e.g. ₹5,000
	 * typed as ₹50,000 or vice-versa).
	 *
