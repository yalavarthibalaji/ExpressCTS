 ...
 * ZK MVC Composer — controls login.zul
 *
 * Fix: onChanging uses InputEvent.getValue() for the currently-typed field,
 *      and reads the committed value from the OTHER field via .getValue().
 *      This ensures role detection works on every keystroke.
 */
public class LoginController extends SelectorComposer<Component> {

    @Wire("#txtUserId")       private Textbox txtUserId;
    @Wire("#txtPassword")     private Textbox txtPassword;

    @Wire("#detectedRoleWrap")  private Div   detectedRoleWrap;
    @Wire("#detectedRoleBox")   private Div   detectedRoleBox;
    @Wire("#detectedRoleIcon")  private Label detectedRoleIcon;
    @Wire("#detectedRoleLabel") private Label detectedRoleLabel;

    @Wire("#loginError")        private Div   loginError;
    @Wire("#loginErrorText")    private Label loginErrorText;
    ...
    /**
     * KEY FIX: onChanging fires BEFORE the textbox value is committed.
     * Cast to InputEvent and call getValue() to get what was JUST typed.
     * Read the OTHER field with .getValue() (already committed).
     */
