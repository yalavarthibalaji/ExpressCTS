...
/**
 * MakerSidebarController.java ZK MVC Composer — controls
 * components/makerSidebar.zul
 *
 * Each nav item click fires a custom event on the sidebar's parent page (the
 * cts-app div in makerDashboard.zul), which the MakerDashboardController
 * listens for.
 *
...
public class MakerSidebarController extends SelectorComposer<Component> {
	...
	@Wire("#navCreateBatch")
	private Div navCreateBatch;
	@Wire("#navLoadCheques")
	private Div navLoadCheques;
	@Wire("#navMakerQueue")
	private Div navMakerQueue;
	@Wire("#navMicrRepair")
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		allNavItems = new Div[] { navDashboard, navCreateBatch, navLoadCheques, navMakerQueue, navMicrRepair,
				navReports };
	}
	...
	/**
	 * Fire a custom event that bubbles up to the root cts-app div where
	 * MakerDashboardController's @Listen annotations catch it.
	 */
