...
/**
 * CheckerController.java Package : com.iispl.controller
 *
 * Panels: panelDashboard - landing summary panelBatchMgmt - Batch Management
 * (step 1) — view batches, verify panelMicrRepair - MICR Repair exception queue
 * — repair + verify panelBatchList - Checker batch queue (default)
 * panelChequeList - Cheques inside a batch panelChequeDetail - Single cheque
 * spot-check panelReports - Reports panelAudit - Audit log
 */
public class CheckerController extends SelectorComposer<Component> {
	...
	// Cannot @Wire across composer boundaries — update via Clients.evalJavaScript
	// instead.
	...
	private void loadMicrRepair() {
		...
		// For the repair table: show all IQA-failed cheques (checker_status =
		// pending/rejected)
		// For the verify table: show cheques with checker_status = 'repaired'
		// (submitted by maker)
	...
	private void populateChequeDetail() {
		...
		fldMakerFlag
				.setValue(safe(activeCheque.getMakerFlag()).isEmpty() ? "— No flag —" : activeCheque.getMakerFlag());
		fldMakerRemarks.setValue(safe(activeCheque.getMakerRemarks()).isEmpty() ? "—" : activeCheque.getMakerRemarks());
		...
		if (decided) {
			lblDecisionResult.setValue("approved".equals(activeCheque.getCheckerStatus()) ? "✅ Spot-check Passed."
		...
		lblImgChequeNum
				.setValue("No: " + activeCheque.getChequeNumber() + "  |  " + safe(activeCheque.getBranchName()));
		...
		lblImgMicr.setValue("|" + safe(activeCheque.getChequeNumber()) + "|  " + safe(activeCheque.getMicrCode())
	...
	// ================================================================
	// NOTE: Logout is now handled by HeaderController (header.zul reusable
	// component).
	...
	public void onVerifyBatch(Event e) {
		...
		try {
			...
			Messagebox.show("Batch " + batchId + " verified and approved.", "Verified", Messagebox.OK,
	...
	public void onSubmitMicrRepair() {
		...
		checkerService.submitMicrRepair(activeMicrChequeId, micrFldMicrCode.getValue().trim(),
				micrFldChequeNum.getValue().trim(), micrFldBankName.getValue().trim(), micrFldIfsc.getValue().trim(),
				remarks);
	...
	public void onReturnToDepositor() {
		...
		String remarks = micrFldRemarks.getValue().trim();
		if (remarks.isEmpty()) {
			Messagebox.show("Remarks required before returning to depositor.", "Required", Messagebox.OK,
	...
	public void onApproveBatch() {
		try {
			...
			Messagebox.show("Batch " + activeBatchId + " APPROVED.", "Approved", Messagebox.OK, Messagebox.INFORMATION);
			showBatchListPanel();
	...
	public void onConfirmReturn() {
		...
		try {
			...
			Messagebox.show("Batch " + activeBatchId + " returned to Maker.", "Returned", Messagebox.OK,
					Messagebox.INFORMATION);
			showBatchListPanel();
	...
	public void onReturnFile() {
		String remarks = txtCheckerRemarks.getValue().trim();
		if (remarks.isEmpty()) {
			Messagebox.show("Please enter remarks before returning the file.", "Remarks Required", Messagebox.OK,
					Messagebox.EXCLAMATION);
			return;
		}
	...
	public void onEscalate() {
		String remarks = txtCheckerRemarks.getValue().trim();
		String escalateNote = remarks.isEmpty() ? "Escalated to supervisor for review." : "Escalated: " + remarks;
		addAudit("SYSTEM", "Cheque #" + activeCheque.getChequeNumber() + " — " + escalateNote);
		Messagebox.show("Cheque #" + activeCheque.getChequeNumber() + " has been escalated for supervisor review.",
	...
	private int findIndex(List<CheckerCheque> list, String chequeId) {
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getChequeId().equals(chequeId))
				return i;
		}
		return -1;
	}
	...
	private String statusSclass(String s) {
		if ("approved".equals(s))
			return "cts-badge cts-badge-green";
		if ("rejected".equals(s))
			return "cts-badge cts-badge-red";
	 ...
	 * CheckerSidebarController. @Wire cannot cross composer boundaries. The
	 * cleanest solution without a shared service is a targeted JS DOM update.
	 */
