...
/**
 * CheckerDao.java DAO Interface — Checker module database operations. Package :
 * com.iispl.dao
 */
public interface CheckerDao {
	...
	/**
	 * Returns all cheques where iqa_status = 'FAIL'. These appear in the MICR
	 * Repair queue.
	 */
	List<CheckerCheque> getMicrRepairCheques();

	/**
	 * Saves corrected MICR data and marks checker_status = 'repaired' so the
	 * checker can verify the correction.
	 */
	void submitMicrRepair(String chequeId, String micrCode, String chequeNum, String bankName, String ifscCode,
			String remarks);

	/**
	 * Resets a cheque checker_status back to 'pending' when checker rejects a MICR
