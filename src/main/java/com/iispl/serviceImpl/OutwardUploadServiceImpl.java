...
public class OutwardUploadServiceImpl implements OutwardUploadService {
	...
	public void processUpload(File zipFile, String uploadedBy) {
		...
		if (chequeList.size() != header.getTotalCheques()) {
			throw new RuntimeException("Cheque count mismatch. Header says: " + header.getTotalCheques()
		...
		for (ChequeXml chequeXml : chequeList) {
			...
			OutwardCheque cheque = buildChequeEntity(chequeXml, batch, extractedFolderPath);
		...
		if (totalAmountFromXml != header.getTotalAmount()) {
			System.out.println("Warning: Total amount mismatch." + " Header: " + header.getTotalAmount() + " Actual: "
					+ totalAmountFromXml);
		}
	...
	private void validateCheque(ChequeXml cheque) {
		...
		if (cheque.getAmountInFigures() <= 0) {
			throw new RuntimeException("Validation failed: AmountInFigures must be greater than 0" + " for cheque: "
					+ cheque.getChequeNumber());
		}
		if (cheque.getMicrCode() == null || cheque.getMicrCode().isEmpty()) {
			throw new RuntimeException(
					"Validation failed: MicrCode is missing" + " for cheque: " + cheque.getChequeNumber());
		}
		if (cheque.getIfscCode() == null || cheque.getIfscCode().isEmpty()) {
			throw new RuntimeException(
					"Validation failed: IfscCode is missing" + " for cheque: " + cheque.getChequeNumber());
		}
	}
	...
	private OutwardCheque buildChequeEntity(ChequeXml chequeXml, OutwardBatch batch, String extractedFolderPath) {
	...
	private String generateChequeId() {
		...
		return "CHQ" + UUID.randomUUID().toString().replace("-", "").substring(0, 13).toUpperCase();
	}
