...
public class BatchUploadServiceImpl implements BatchUploadService {
    ...
    public BatchUploadResult processBatchUpload(File   zipFile,
        ...
        // If this exact filename was already uploaded by this maker (and not rejected),
        // block the reupload — same file shouldn't create a second batch.
        if (originalFileName != null && !originalFileName.trim().isEmpty()) {
            boolean isDuplicate = batchDao.existsByFileNameAndMaker(
                originalFileName.trim(), makerId);
            if (isDuplicate) {
                return BatchUploadResult.failure(
                    "This file (\"" + originalFileName + "\") has already been uploaded. "
                    + "Please upload a different file or reject the existing batch first.");
            }
        }
    ...
    private OutwardBatch buildBatchEntity(String batchId,
        ...
        batch.setStatus(status);
        batch.setRepairStatus(repairStatus);
        batch.setFilePath(originalFileName);   // store original ZIP name for duplicate detection
        batch.setCreatedAt(LocalDateTime.now());
