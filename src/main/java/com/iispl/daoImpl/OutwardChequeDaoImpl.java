...
public class OutwardChequeDaoImpl implements OutwardChequeDao {
    ...
    // Used by MakerDashboardService to save maker-verified data back to DB
    @Override
    public void updateCheque(OutwardCheque cheque) {
        Transaction transaction = null;
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            transaction = session.beginTransaction();
            session.merge(cheque);
            transaction.commit();
            System.out.println("Cheque updated: " + cheque.getChequeNumber());
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
            }
            e.printStackTrace();
            throw new RuntimeException("Failed to update cheque: " + e.getMessage());
        }
    }
    ...
    public List<OutwardCheque> findAllByBatchId(Long batchId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<OutwardCheque> query = session.createQuery(
                "FROM OutwardCheque WHERE batch.id = :batchId", OutwardCheque.class);
    ...
    public OutwardCheque findByChequeId(String chequeId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<OutwardCheque> query = session.createQuery(
                "FROM OutwardCheque WHERE chequeId = :chequeId", OutwardCheque.class);
