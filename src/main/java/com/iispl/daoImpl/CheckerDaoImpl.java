...
/**
 * CheckerDaoImpl.java
 * DAO Implementation — in-memory data for testing.
 * Package : com.iispl.daoImpl
 *
 * TEST DATA:
 *   1 batch  → BATCH15042025001 (CHECKER_PENDING)
 *   1 cheque → CHQ001 (State Bank of India, Rajesh Kumar, ₹2,50,000)
 *
 * When you click "Open Batch" → you see 1 cheque row.
 * When you click "View" on that row → full detail screen opens.
 * This lets you verify the entire flow end to end.
 *
 * MICR queue: CHQ001 is IQA PASS, so MICR queue is empty for now.
 * To test MICR: change "PASS" to "FAIL" in addCheque below.
 */
public class CheckerDaoImpl implements CheckerDao {
    ...
    private static void initData() {
        ...
        CheckerBatch b1 = new CheckerBatch(
            "BATCH15042025001", 1, 250000L, "CHECKER_PENDING"
        );
    ...
    private static void addCheque(
            String chequeId,    String txnId,       String chequeNo,
            String bank,        String branch,      String ifsc,    String micr,
            String drawerName,  String drawerAcc,
            String chequeStatus,String bounceReason,
            String chequeDate,  String presentDate,
            long amount,        String amountWords,
            String payee,       String depositorAcc,
            String iqa,         String batchId,
            String makerFlag,   String makerRemarks) {
    ...
    public void submitMicrRepair(String chequeId, String micrCode, String chequeNum,
        ...
        for (CheckerCheque c : CHEQUES) {
            if (chequeId != null && chequeId.equals(c.getChequeId())) {
                if (!micrCode.isEmpty())  c.setMicrCode(micrCode);
                if (!chequeNum.isEmpty()) c.setChequeNumber(chequeNum);
                if (!bankName.isEmpty())  c.setBankName(bankName);
                if (!ifscCode.isEmpty())  c.setIfscCode(ifscCode);
