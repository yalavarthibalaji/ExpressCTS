...
public class OutwardBatchDaoImpl implements OutwardBatchDao {
	...
	public OutwardBatch findByBatchId(String batchId) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<OutwardBatch> query = session.createQuery("FROM OutwardBatch WHERE batchId = :batchId",
					OutwardBatch.class);
	...
	public int countBatchesTodayForBranch(String branchCode, String date) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			Query<Long> query = session.createQuery("SELECT COUNT(b) FROM OutwardBatch b "
					+ "WHERE b.branchCode = :branchCode " + "AND CAST(b.scannedAt AS date) = CAST(:date AS date)",
					Long.class);
